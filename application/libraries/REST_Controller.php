...
//        header("Access-Control-Allow-Headers: X-API-KEY,TOKEN-KEY, Origin, X-Requested-With, Content-Type, Accept, Access-Control-Request-Method");
...
//        }
    ...
    protected function _check_login($username = NULL, $password = FALSE)
    {
        ...
        $new = array();
        $authusers = $this->Auth_mobileUsers_Model->get_users();
        if(!empty($authusers)){
            foreach ($authusers as $users){
                $new[$users["UserName"]] = $users["Password"];
            }
        }
        $valid_logins = $new;
    ...
    protected function _prepare_basic_auth()
    {
       ...
       if( $_SERVER['HTTP_TOKEN_KEY']=='null') {
           $username = $request->PHP_AUTH_USER;//$this->input->request('PHP_AUTH_USER');
       }
        ...
        if( $_SERVER['HTTP_TOKEN_KEY']=='null') {
            ...
            if ($username !== NULL) {
                ...
                $password = MD5($request->PHP_AUTH_PW);//MD5($this->input->request('PHP_AUTH_PW'));
        ...
        if ($this->_check_login($username, $password) === FALSE && (!$_SERVER['HTTP_TOKEN_KEY'] || $_SERVER['HTTP_TOKEN_KEY']=='null' ))
        {
            $this->_force_login();
        }
    }
